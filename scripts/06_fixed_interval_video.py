...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/frame_scores/fixed_interval"),
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Run YOLO on every k-th frame. Default: 10.",
    )
    parser.add_argument(
        "--conf",
        type=float,
...
def print_run_settings(args, device):
    """Print the main fixed-interval inference settings."""
    print("Starting fixed-interval YOLO video inference")
...
def skipped_row(frame_index, timestamp_sec):
    """Build a CSV row for an unprocessed frame."""
    return {
        "frame_index": frame_index,
        "timestamp_sec": f"{timestamp_sec:.6f}",
        "max_fire_conf": "0.000000",
        "max_smoke_conf": "0.000000",
        "event_score": "0.000000",
        "num_fire_detections": 0,
        "num_smoke_detections": 0,
        "processed": False,
    }
...
def main():
    ...
    if args.interval <= 0:
        raise SystemExit("Error: --interval must be a positive integer.")
    ...
    csv_path = args.output_dir / f"{args.video.stem}_fixed_interval_{args.interval}_scores.csv"
    summary_path = args.output_dir / f"{args.video.stem}_fixed_interval_{args.interval}_summary.json"
    ...
    if args.save_video:
        annotated_video_path = Path("videos/output") / (
            f"{args.video.stem}_fixed_interval_{args.interval}.mp4"
        )
    ...
    while True:
        ...
        if frame_index % args.interval == 0:
            ...
            rows.append(skipped_row(frame_index, timestamp_sec))
            if video_writer is not None:
                video_writer.write(frame)

        frame_index += 1
        if frame_index % 100 == 0:
            print(f"  scanned {frame_index} frames, processed {processed_frames}")
    ...
    total_scanned_frames = len(rows)
    skipped_frames = total_scanned_frames - processed_frames
    effective_fps = processed_frames / runtime_sec if runtime_sec > 0 else 0.0
    frame_reduction_percent = (
        skipped_frames / total_scanned_frames * 100 if total_scanned_frames > 0 else 0.0
    )
    summary = {
        "method": "fixed_interval",
        "video_path": str(args.video),
        "model_path": str(args.model),
        "total_frames": total_frames if total_frames else total_scanned_frames,
    ...
    print("\nFixed-interval baseline complete")
